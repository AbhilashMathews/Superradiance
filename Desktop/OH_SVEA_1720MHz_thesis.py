...
#----------------calculating NE, PpE, and EpE----------------
        ...
        while i < (len(t) - 1):
            ...
            theta0_i = theta0_mean#np.abs(np.random.normal(theta0_mean, sigma_theta0))

#        Initial conditions   
            Pp[k,0] = 0.5*np.sin(theta0_k)#*np.sin(z[:]/(0.001*Lp))#(np.e**((-(z[:])**2.)/(0.001*L_mean)**2.))
            N[k,0] = 0.5*np.cos(theta0_k)#the above lines commented out is just one way to alter initial conditions
            if i == 0:  
                EpM[k,0] = 0.
#        Boundary conditions
